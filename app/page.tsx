...
const Landing = () => {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < 14; i++) {
            ...
            const velocityX = (Math.random() * (0.5 - 2) + 0.1) * (Math.random() < 0.5 ? 0.1 : -0.1);
            const velocityY = (Math.random() * (0.5 - 2) + 0.1) * (Math.random() < 0.5 ? 0.1 : -0.1);
        ...
        const handleCollision = (circleA: CircleData, circleB: CircleData) => {
            ...
            velA.x = ((circleA.size - circleB.size) * velA.x + 2 * circleB.size * velB.x) / (circleA.size + circleB.size);
            velB.x = vxTotal + velA.x;

            // Update positions to avoid sticking
            const absV = Math.abs(velA.x) + Math.abs(velB.x);
            const overlap = (circleA.size + circleB.size) / 2 - Math.abs(posA.x - posB.x);
    ...
    return (
        <div className="min-h-screen  overflow-x-hidden">
            ...
            <section className=" bg-gradient-to-b from-black/50 from-90%   to-black backdrop-blur-sm  text-center relative">
                ...
                <LandingNav />
                <div className="2xl:mx-auto 2xl:max-w-[1400px] max-w-[640px] mx-auto z-10  flex flex-col items-center  min-h-[calc(100vh-125px)] justify-center  px-4"
                    ref={heroRef}
                >
                    <motion.h1 style={{ opacity: 0 }} animate={{ opacity: 1, y: -10 }} className={`${katibeh.className} text-9xl text-white`}>
                        Applicat
                    </motion.h1>
