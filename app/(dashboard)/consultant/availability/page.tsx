...
const ConsultantPage = ({ params: { user } }: { params: { user: string } }) => {
  ...
  const [defaultAvailabilityData, setDefaultAvailabilityData] = useState<
    TAvailability[]
  >([]);
  ...
  const handleDayClick = (day: Date, modifiers: any) => {
    ...
    setValue((prev: Date[]) =>
      modifiers?.selected
        ? prev.filter(
            (_, i: number) =>
              prev.findIndex((prevDay) => isSameDay(day, prevDay)) !== i
          )
        : [...prev, day]
    );
  };
    ...
    if (userSession) {
      ...
      fetch(`/api/auth/session`)
        ...
        .then((res) => {
          ...
          fetch(`/api/availability/?consultantId=${consultantId}`)
            ...
            .then((availabilityFromDb) => {
              ...
              if (availabilities) setDefaultAvailabilityData(availabilities);
            });
        });
          ...
          <div className="flex justify-start sm:justify-center lg:justify-start w-full md:w-auto">
            {value.length < 1 ? (
              ...
              <AvailabilityForm
                ...
                defaultValue={
                  defaultAvailabilityData.find(
                    (availability: TAvailability) => {
                      const storedDateString = new Date(
                        availability.date
                      ).toISOString();
                      // every selected date in iso string format array
                      const LastSelectedDateString = new Date(
