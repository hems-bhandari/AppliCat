...
const ConsultantDialog = ({
    ...
    const handleNext = async () => {
        if (stage === 0) {
            if (!date.justDate || !date.dateTime || !authUserSession.data?.user)
                return;

            // filtering the active availability.

            if (!availabilities || !date.justDate) return;
            const selectedAvailability = Object.keys(availabilities).find(
                (availableDate) =>
    ...
    const submitConfirmation = async (data: ConfirmationFormValues) => {
        ...
        const confirmedSession = await confirmPendingSession({
            ...
            email: data.email,
            consultantEmail: (consultantData as any)?.email,
        });
